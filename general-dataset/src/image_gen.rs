...
pub fn flips<P: Pixel<Subpixel = u8> + Send + Sync + 'static>(
    iter: impl ParallelIterator<Item = ImageBuffer<P, Vec<u8>>>,
) -> impl ParallelIterator<Item = ImageBuffer<P, Vec<u8>>> {
    iter.flat_map(|img| {
        [
...
pub fn rotations(
    iter: impl ParallelIterator<Item = Image>,
    count: usize,
    default: Rgb<u8>,
) -> impl ParallelIterator<Item = Image> {
    iter.flat_map(move |src_img| {
            ...
            .map(move |_| {
                ...
                warp(
                    ...
                    &Projection::translate(
                        ...
                        rng.sample(height_distr)
                            .clamp(-(height as f32), height as f32),
                    ),
                    Interpolation::Bilinear,
                    default,
                )
...
pub fn noises(
    iter: impl ParallelIterator<Item = Image>,
    count: usize,
    std_devs: impl IntoParallelIterator<Item = f64> + Send + Sync + Clone,
) -> impl ParallelIterator<Item = Image> {
    iter.flat_map(move |src_img| {
        let src_img2 = src_img.clone();
        ...
        (0..count)
            ...
            .flat_map(move |_| {
                ...
                std_devs
                    .clone()
                    .into_par_iter()
                    .map(move |std_dev| gaussian_noise(&src_img, 0.0, std_dev, random()))
            })
            .chain(Some(src_img2))
    })
}
