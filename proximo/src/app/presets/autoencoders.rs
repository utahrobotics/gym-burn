...
use general_models::{
    ...
    composite::{
        autoencoder::{
            ...
            vae::{VariationalEncoderModel, VariationalEncoderModelConfig},
        },
        image::{
...
use crate::trainable_models::apply_gradients::{
    ApplyGradients,
    autoencoder::{
        AutoEncoderModelPlan, AutoEncoderModelPlanConfig, VariationalEncoderModelPlan,
        VariationalEncoderModelPlanConfig,
    },
    image::{
        Conv2dLinearModelPlan, Conv2dLinearModelPlanConfig, LinearConvTranspose2dModelPlan,
...
pub enum ImageAutoEncoder<B: Backend> {
    Normal(AutoEncoderModel<B, Conv2dLinearModel<B>, LinearConvTranspose2dModel<B>>),
    Vae(
        AutoEncoderModel<
            B,
...
pub enum ImageAutoEncoderConfig {
    Normal(AutoEncoderModelConfig<Conv2dLinearModelConfig, LinearConvTranspose2dModelConfig>),
    Vae(
...
pub enum ImageAutoEncoderPlan<B: AutodiffBackend> {
    ...
    Vae(
        AutoEncoderModelPlan<
            VariationalEncoderModelPlan<B, Conv2dLinearModelPlan<B>>,
            LinearConvTranspose2dModelPlan<B>,
        >,
    ),
}
...
pub enum ImageAutoEncoderPlanConfig {
    ...
    Vae(
        AutoEncoderModelPlanConfig<
            VariationalEncoderModelPlanConfig<Conv2dLinearModelPlanConfig>,
            LinearConvTranspose2dModelPlanConfig,
        >,
    ),
}
...
impl<B: AutodiffBackend> ApplyGradients<B> for ImageAutoEncoder<B> {
    ...
    fn config_to_plan(config: Self::PlanConfig) -> Self::Plan {
        match config {
            ImageAutoEncoderPlanConfig::Normal(x) => {
                ImageAutoEncoderPlan::Normal(AutoEncoderModel::<
                    B,
                    Conv2dLinearModel<B>,
                    LinearConvTranspose2dModel<B>,
                >::config_to_plan(x))
            }
            ImageAutoEncoderPlanConfig::Vae(x) => {
                ImageAutoEncoderPlan::Vae(AutoEncoderModel::<
                    B,
                    VariationalEncoderModel<B, Conv2dLinearModel<B>>,
                    LinearConvTranspose2dModel<B>,
                >::config_to_plan(x))
    ...
    fn apply_gradients(
        ...
        match self {
            ImageAutoEncoder::Normal(x) => x.apply_gradients(lr, grads, {
                let ImageAutoEncoderPlan::Normal(plan) = plan else {
                    panic!("Incorrect model plan");
                };
                plan
            }),
            ImageAutoEncoder::Vae(x) => x.apply_gradients(lr, grads, {
                let ImageAutoEncoderPlan::Vae(plan) = plan else {
                    panic!("Incorrect model plan");
                };
                plan
            }),
