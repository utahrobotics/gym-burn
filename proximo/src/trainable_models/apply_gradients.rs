use burn::{nn::Linear, optim::GradientsParams, tensor::backend::AutodiffBackend};
use general_models::{common::Norm, linear::LinearModel};
...
impl<B: AutodiffBackend> ApplyGradients<B> for LinearModel<B> {
    ...
    fn apply_gradients(&mut self, lr: f64, grads: &mut  <B as AutodiffBackend>::Gradients, plan: &mut Self::Plan) {
        self.iter_layers(|mut linear, mut norm| {
