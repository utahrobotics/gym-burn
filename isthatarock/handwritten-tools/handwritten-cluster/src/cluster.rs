...
pub fn cluster(points: Vec<[f64; 3]>, min_points: usize) -> ClusteringResult {
    let mut distances: Vec<_> = points
        ...
        .map(|a| {
            let mut distances: Vec<_> = points
                .par_iter()
                .copied()
                .map(|b| (a[0] - b[0]).powi(2) + (a[1] - b[1]).powi(2) + (a[2] - b[2]).powi(2))
                .collect();
    ...
    let classifications = dbscan::cluster(
        eps,
        min_points,
    ...
    for (classification, point) in classifications.into_iter().zip(points) {
        match classification {
            Classification::Core(id) | Classification::Edge(id) => match classified.entry(id) {
