use std::{collections::VecDeque, env, num::NonZeroUsize, path::{Path, PathBuf}, sync::{LazyLock, OnceLock, atomic::{AtomicUsize, Ordering}}, time::{Duration, Instant}};
...
pub(crate) fn hash_tensor(tensor: &FloatTensor<TrackingBackend>) -> [u8; 32] {
    ...
    let data = tensor.primitive.client.read_one(tensor.primitive.handle.clone());
...
impl TrackedTensorBuilder {
    pub(crate) fn finish_ref(self, tensor: &FloatTensor<TrackingBackend>) {
        TRACKED_TENSORS.lock().push(
            TrackedTensor { from_hash: self.from_hash, to_hash: Some(hash_tensor(tensor)), operation: self.operation, extra_data: self.extra_data, time: get_time() }
        );
    }
    ...
    pub(crate) fn finish_no_tensor(self) {
        TRACKED_TENSORS.lock().push(
            TrackedTensor { from_hash: self.from_hash, to_hash: None, operation: self.operation, extra_data: self.extra_data, time: get_time() }
        );
    }
}

pub(crate) fn finish_iter(builders: impl IntoIterator<Item = TrackedTensorBuilder>, to_hash: Option<[u8; 32]>) {
    let mut guard = TRACKED_TENSORS.lock();
    let time = get_time();
    guard.extend(
        builders.into_iter()
            .map(|builder| {
                TrackedTensor { from_hash: builder.from_hash, to_hash, operation: builder.operation, extra_data: builder.extra_data, time }
            })
    );
}
